...
const FacebookAuthButton = () => {
    const [facebookClientId, setFacebookClientId] = useState<string | null>(null);
    const auth = useAuthentication();
    const { addErrorAlert } = useAlertQueue();
    ...
    useEffect(() => {
        (async () => {
            if (facebookClientId !== null) return;

            const { data, error } = await auth.client.GET("/users/facebook/client-id");
            if (error) {
                addErrorAlert(error);
            } else {
                setFacebookClientId(data.client_id);
            }
        })();
    }, [facebookClientId]);
    ...
    const responseFacebook = async (response) => {
        if (response.status === 'unknown') {
            addErrorAlert('Facebook login failed. Please try again.');
            return;
        }
        ...
        try {
            const backendResponse = await auth.client.POST("/users/facebook/login", {
                body: {
                    token: accessToken,
                },
            });
            ...
            addErrorAlert('Backend authentication failed');
            console.error("Backend login failed:", error);
        }
    };


    return facebookClientId === null ? (
        <Button
            variant={"outline"}
            size={"lg"}
            className="w-full hover:bg-gray-100 dark:hover:bg-gray-600"
