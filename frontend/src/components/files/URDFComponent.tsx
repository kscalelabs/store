...
const getCollisions = (camera: THREE.Camera, robot: THREE.Object3D | null, mouse: THREE.Vector2) => {
    if (!robot) return [];
...
const LoadModel: React.FC<LoadModelProps> = ({ filepath }) => {
    ...
    const highlightLinkGeometry = useCallback((m: THREE.Object3D | null, revert: boolean) => {
        if (!m) return;
