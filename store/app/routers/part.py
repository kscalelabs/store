...
from store.app.model import Image, Listing, User
...
async def dump_parts(crud: Annotated[Crud, Depends(Crud.get)]) -> list[Listing]:
...
async def get_part(part_id: str, crud: Annotated[Crud, Depends(Crud.get)]) -> Listing | None:
...
async def edit_part(
    ...
    if user.id != part_info.owner:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You do not own this part")
    part["owner"] = user.id
    await crud._update_item(part_id, Listing, part)
    return True
